package main

import (
	"os"
	"testing"
	"encoding/json"
	"strconv"

	"github.com/google/uuid"
	"github.com/stretchr/testify/assert"
	"github.com/hyperledger/fabric-chaincode-go/shimtest"

	"github.com/hyperledger/fabric-contract-api-go/contractapi"
)

func  TestLoanBuyerContract(t *testing.T) {
	os.Setenv("MODE","TEST")
	
	assert := assert.New(t)
	uid := uuid.New().String()

	cc, err := contractapi.NewChaincode(new(LoanBuyerContract))
	assert.Nil(err, "error should be nil")

	stub := shimtest.NewMockStub("TestStub", cc)
	assert.NotNil(stub, "Stub is nil, TestStub creation failed")

	// - - - test LoanBuyerContract:Put function - - - 
	putResp := stub.MockInvoke(uid,[][]byte{
		[]byte("LoanBuyerContract:Put"),
		[]byte("1"),
		[]byte("LoanBuyerName"),
		[]byte("BuyerCategory"),
		[]byte("1"),
		[]byte("HqAddress"),
		[]byte("Location"),
		[]byte("LocationLat"),
		[]byte("LocationLong"),
	})
	assert.EqualValues(OK, putResp.GetStatus(), putResp.GetMessage())
	

	// - - - test LoanBuyerContract:Get function - - - 
	testID := "1"
	getResp := stub.MockInvoke(uid, [][]byte{
		[]byte("LoanBuyerContract:Get"),
		[]byte(testID),
	})
	assert.EqualValues(OK, getResp.GetStatus(), getResp.GetMessage())
	assert.NotNil(getResp.Payload, "getResp.Payload should not be nil")
	
	loanBuyerObj := new(LoanBuyerObj)
	err = json.Unmarshal(getResp.Payload, loanBuyerObj)
	assert.Nil(err, "json.Unmarshal error should be nil")
	assert.NotNil(loanBuyerObj, "loanBuyerObj should not be nil")

	retrievedID := strconv.Itoa(loanBuyerObj.LoanBuyerID)
	assert.EqualValues(testID, retrievedID, "testID and retrievedID mismatch")
}